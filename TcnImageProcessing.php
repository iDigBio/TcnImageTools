			...
			if(count($aArr) == 2){
				...
				if(!file_exists($this->targetPathBase.$acro.'/'.$collName)){
					if(!mkdir($this->targetPathBase.$acro.'/'.$collName)){
						$this->logOrEcho("ERROR: unable to create new folder (".$this->targetPathBase.$acro.'/'.$collName.") \n");
					}
				}
				
				//Lets start processing folder
				$this->logOrEcho('Starting image processing: '.$acro.'/'.$collName."\n");
				if(file_exists($this->targetPathBase.$acro.'/'.$collName)){
					$this->processFolder($acro.'/'.$collName.'/');
				}
				$this->logOrEcho('Image upload complete for '.$acro.'/'.$collName."\n");
				$this->logOrEcho("-----------------------------------------------------\n\n");
			...
			if($occId || !$this->dbMetadata){
				...
				if($webImgCreated){
					...
					if($this->recordImageMetadata(($this->dbMetadata?$occId:$specPk),$targetFolder.$targetFileName,$tnUrl,$lgUrl)){
						if($this->keepOrig){
							if(file_exists($this->sourcePathBase.$pathFrag.$fileName)) rename($this->sourcePathBase.$pathFrag.$fileName,$this->sourcePathBase.$pathFrag.$fileName.".orig");
						} else {
	...
	private function processMetadataFile($filePath){
		...
		if($fh){
			...
			$this->logOrEcho("\tMetadata file loaded \n");
                        if($this->keepOrig){
	                        if(file_exists($this->sourcePathBase.$pathFrag.$fileName)) rename($filePath,$filePath.".orig");
                        } else {
	                        if(!unlink($filePath)){
